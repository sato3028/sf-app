<template>
    <div class="filter-panel">
        ...
        <div class="input-group">
            <label for="host_rank">
                ルーム作成者のランク帯:
                <span :style="{ color: getRankColor(filters.host_rank[0]) }">{{ getRankText(filters.host_rank[0])
                    }}</span>
                -
                <span :style="{ color: getRankColor(filters.host_rank[1]) }">{{ getRankText(filters.host_rank[1])
                    }}</span>
            </label>
            <v-range-slider v-model="filters.host_rank" :max="25000" :min="1" :step="1" hide-details
                track-color="#d6d0da" thumb-color="#6b4ef5" class="input-field"></v-range-slider>
            ...
            <div v-if="filters.host_rank[1] === 25000" class="mr-slider">
                ...
                <v-range-slider v-model="filters.host_mr_range" :max="2500" :min="1000" :step="1" hide-details
                    track-color="#d6d0da" thumb-color="#6b4ef5" class="input-field"></v-range-slider>
            </div>
        </div>
        ...
        <div class="input-group">
            ...
            <FilterCharacterModal :show="showHostCharacterModal" :characters="characters"
                :selected-characters="filters.host_characters" @close="showHostCharacterModal = false"
        ...
        <div class="input-group">
            <label for="rank_range">
                募集ランク帯:
                <span :style="{ color: getRankColor(filters.rank_range[0]) }">{{ getRankText(filters.rank_range[0])
                    }}</span>
                -
                <span :style="{ color: getRankColor(filters.rank_range[1]) }">{{ getRankText(filters.rank_range[1])
                    }}</span>
            </label>
            <v-range-slider v-model="filters.rank_range" :max="25000" :min="1" :step="1" hide-details
                track-color="#d6d0da" thumb-color="#6b4ef5" class="input-field"></v-range-slider>
            ...
            <div v-if="filters.rank_range[1] === 25000" class="mr-slider">
                ...
                <v-range-slider v-model="filters.requested_mr_range" :max="2500" :min="1000" :step="1" hide-details
                    track-color="#d6d0da" thumb-color="#6b4ef5" class="input-field"></v-range-slider>
            </div>
        </div>
        ...
        <div class="input-group">
            ...
            <FilterCharacterModal :show="showRequestedCharacterModal" :characters="characters"
                :selected-characters="filters.requested_characters" @close="showRequestedCharacterModal = false"
        ...
        <div class="input-group">
            ...
            <div v-if="attributes && attributes.length > 0" class="checkbox-list">
                <div v-for="attribute in attributes" :key="attribute.id"
                    ...
                    <input type="checkbox" :id="'attribute' + attribute.id" v-model="filters.attributes"
                        :value="attribute.id" />
